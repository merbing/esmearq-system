...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // 1. Atualizar o status da solicitação para 'Aprovado' e definir data_limite_edicao para 2 horas
    $sql_atualizar_status = "UPDATE solicitacoes_edicao 
                             SET status = 'Aprovado', id_aprovador = $funcionario_id,
                                 data_solicitacao = NOW(), 
                                 data_limite_edicao = DATE_ADD(NOW(), INTERVAL 2 HOUR) 
    ...
    // 2. Registrar a atividade da solicitação aprovada
    $descricao_atividade = "Solicitação Aprovada - ID: $id_solicitacao";
    $sql_inserir_atividade = "INSERT INTO atividades 
                              (descricao, status, data_inicio, funcionario_id, agencia_id, id_alterado)
    ...
    $notification_message = "Sua solicitação foi aprovada.";
    ...
    if ($conn->query($sql_insert_notification) === TRUE) {
        ...
        $mensagem_sucesso = urlencode("Solicitação aprovada com sucesso.");
